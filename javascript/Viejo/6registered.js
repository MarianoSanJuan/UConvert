...
function cargaProducto(usuarioLogueado){
    console.log("entro")
    ...
    let formOk = true
        if (idProductoIngresado<4){
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Same ID Product as Products Examples. Please check above',

            })
            return formOk = false
        }
            if (!idProductoIngresado || !precio || !proveedor || !stock || !nombre ){
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
