...
//Producto
class Producto {
    constructor (id, precio, proveedor, stock,nombre){
        this.id = id;
        this.precio = precio;
...
class Venta {
    ...
    calcularPrecioConImpuestos () {
        return this.producto.precio * 1.21;
    }
...
do {
    ...
    arrayProductos.map( producto => mensajeProductos += `ID: ${producto.id}. Nombre: ${producto.nombre}.Precio $: ${producto.precio}.Proveedor: ${producto.proveedor}.Stock: ${producto.stock} \n`);

    do {
        let nroProductoElegido = parseInt(prompt(`Qué producto desear facturar?\n Ingresar ID. Estos son los creados previamente\n ${mensajeProductos}`));
        let productoElegido = arrayProductos.find ( producto => producto.id === nroProductoElegido);
        ...
        sessionStorage.setItem("Venta:"+contadorVenta,clienteJSON);
