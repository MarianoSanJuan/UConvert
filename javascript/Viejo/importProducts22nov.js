...
$(document).ready(()=>{
    ...
    $("#showProductsBtn").click(() => {
        $.getJSON(jsonProductsUrl, function (respuesta, estado) {
            ...
            arrayProductosPreCargados = productosPreCargados.map ( item => new Producto (item.idProducto ,item.precio, item.proveedor, item.stock, item.nombre));
            console.log(arrayProductosPreCargados);
